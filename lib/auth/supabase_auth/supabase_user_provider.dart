...
class KindergartenSupabaseUser extends BaseAuthUser {
  KindergartenSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> kindergartenSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = KindergartenSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
